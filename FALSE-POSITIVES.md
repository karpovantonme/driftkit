...
| 96 | rstdrift | Reading only `:param` and not the epytext `@param` | Twisted: 8 callables seen instead of 2662, the project looked clean | Both sigils in one pattern, and the callable count printed next to the finding count |
| 97 | rstdrift | `*args` passthrough: `Image.eval(image, *args)` documents `:param function:` | 1 of 2 on Pillow, and removing it would have made the docs worse | `*args` makes the finding soft rather than skipping it; `**kwargs` is still skipped outright |
| 98 | rstdrift | A class documenting its **attributes** under `:param:` | 2 of 18 on certbot (`Addr` takes `tup`, documents `addr` and `port`), 16 of 30 on qutebrowser | Recorded rather than fixed: the field alone cannot tell an attribute from an argument. Named in the blind spots |
| 99 | rstdrift | Editing by text rather than by the reported line | salt: `:param rdata:` occurs 9 times, 6 of them correct; `:param name: specify a specific module to list.` matched 4 times, 2 of them as a prefix of a longer correct line | Fixes are applied by line number from the tool's own output. In this dialect the fields are short and repeat across a file |
| 100 | rstdrift | Deleting the last field in a section leaves the header | qutebrowser CI went red on D414 after `_gather_crash_info` lost all three of its fields | After a deletion, an `Args:`/`:param:` header with nothing under it is dropped too |
