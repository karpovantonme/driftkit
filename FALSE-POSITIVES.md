...
| 103 | supportdrift | The `include:` block of a CI matrix was not read, so a version tested only there counted as untested | 1 on python-fire: the declared minimum 3.7 lives in `include` | Read the key inside inline mappings too |
| 104 | supportdrift | A matrix entry more specific than the claim: declared 3.14, matrix runs `3.14.0-rc.2` | 1 on python-fire | A claimed X.Y is covered by any X.Y.Z |
| 105 | linkdrift | No `Accept` header, so a single-page site cannot tell what is being asked for and answers 404 to a live page | 26 of 35 on comprehensive-rust. The first diagnosis -- "these hosts block robots by user agent" -- was wrong, and measuring beat guessing | Send the browser's `Accept` string |
| 106 | assertdrift, ifacedrift | `sys.exit` on a missing dependency: in a sweep it killed every check that would have followed, and a dead tool reads in a log exactly like a clean one | Whole sweeps lost, silently | Report "NOT RUN", return exit code 2, keep going. Their tests skip instead of failing |
| 100 | doxdrift | A constructor with a member-initializer list: the declaration plus the initializers runs past the 900-character cut, the match slides forward and the comment is compared against the NEXT declaration | 8 from one docblock on PCL, the largest single cluster; the same shape on votca | A declaration also ends at the `:` that opens the initializer list |
| 101 | doxdrift | A parameter written as a bare user type, `void compute (PointCloudOut &)` | 1 on PCL, and the keyword list did not cover it | One word is a type whether or not it is a keyword |
| 102 | doxdrift | `using S = void(const pcl::PointCloud<PointSource>&, ...)`: the last word of a decorated type is a template argument, not a name | 2 on PCL, reported as parameter `PointSource` | Anything ending in `&`, `*` or `>` has no name to take |
